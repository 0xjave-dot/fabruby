...
function makeSessionId() {
  const existing = sessionStorage.getItem("fabruby-visit-session");
  if (existing) return existing;

  const next = `visit-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
  sessionStorage.setItem("fabruby-visit-session", next);
  return next;
}
...
export async function recordVisit(path: string, userId?: string) {
  ...
  const dedupeKey = `fabruby-visit:${sessionStorage.getItem("fabruby-visit-session") ?? "anon"}:${path}`;
