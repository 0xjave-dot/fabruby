...
import type { ColorEntry } from "../data/colorOfTheDay";
import { colorOfTheDaySchedule as seedColorSchedule } from "../data/colorOfTheDay";
...
export type CatalogColorEntry = ColorEntry & {
  id: string;
  index: number;
  active?: boolean;
  updatedAt?: unknown;
...
export function useColorSchedule(): { schedule: CatalogColorEntry[]; loading: boolean } {
  const remote = useCollectionSnapshot<CatalogColorEntry>("colorOfTheDay");

  const schedule = useMemo(() => {
    const seed = seedColorSchedule.map((entry, index) => ({
      id: `slot-${index}`,
      index,
      ...entry,
    }));
...
export function getTodaysColorEntryFromSchedule(schedule: CatalogColorEntry[], date = new Date()) {
...
export async function saveColorSchedule(schedule: Array<Omit<CatalogColorEntry, "id" | "index">>) {
  const batch = writeBatch(db);

  schedule.forEach((entry, index) => {
    const id = `slot-${index}`;
    batch.set(
      doc(db, "colorOfTheDay", id),
      {
        id,
        index,
        ...entry,
